...
const Services = () => (
  <>
    <Seo
      title={`${company.short} Services | Procurement, Installation, Support`}
      description="Explore procurement, installation, training, maintenance, and logistics services for healthcare and laboratory customers in Qatar."
      path="/services"
    />
    ...
    <section className="py-20">
      <div className="container mx-auto container-px grid md:grid-cols-2 lg:grid-cols-3 gap-7">
        {services.map((service, index) => {
          ...
          return (
            <div key={service.title} className="group rounded-2xl border bg-card p-8 hover:shadow-elegant hover:-translate-y-1 transition-all duration-300">
              ...
              <h3 className="font-display font-semibold text-xl text-primary">{service.title}</h3>
      ...
      <div className="container mx-auto container-px">
        ...
        <div className="mt-14 grid md:grid-cols-4 gap-6">
          {[
            ...
            <div key={phase.step} className="relative rounded-2xl bg-card border p-7 shadow-soft">
              <div className="text-5xl font-display font-bold text-accent/30">{phase.step}</div>
              <h3 className="mt-2 font-display font-semibold text-lg text-primary">{phase.title}</h3>
              <p className="mt-2 text-sm text-muted-foreground">{phase.text}</p>
            </div>
          ))}
        </div>
      </div>
